...
export function getMimeType(ext: string): string {
  ...
  const found = mimeTypes.find((m) => m.ext === normalized)
  return found?.mimeType || 'application/octet-stream'
}
...
export function getExtension(mimeType: string): string {
  const found = mimeTypes.find((m) => m.mimeType === mimeType)
  return found?.ext || ''
}
...
export function isDocumentMime(mimeType: string): boolean {
  ...
  return documentTypes.some((t) => mimeType.startsWith(t))
}
