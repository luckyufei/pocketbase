...
To authenticate with password you must enable the *Identity/Password* auth collection option (see also [Web API reference](/api/records#auth-with-password)).
...
To authenticate with email code you must enable the *One-time password (OTP)* auth collection option (see also [Web API reference](/api/records#auth-with-otp)).
...
Superusers have the option to generate tokens and authenticate as anyone else via the [Impersonate endpoint](/api/records#impersonate).
...
PocketBase doesn't have a dedicated token verification endpoint, but if you want to verify an existing auth token from a 3rd party app you can send an [Auth refresh](/api/records#auth-refresh) call, aka. `pb.collection("users").authRefresh()`.
