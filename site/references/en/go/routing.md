...
The `e.BindBody` argument must be a pointer to a struct or `map[string]any`. The following struct tags are supported (*the specific binding rules and which one will be used depend on the request Content-Type*):
...
You can read and bind the request body to a struct using `BindBody`:
...
// Real client IP (respects trusted proxy headers)
realIP := e.RealIP()
```
...
PocketBase automatically converts errors to safe JSON API responses. In development mode or logs, the original error details are shown.
...
```go
return e.Error(400, "Validation failed", map[string]validation.Error{
    "title": validation.NewError("invalid_title", "Title is required"),
    "email": validation.NewError("invalid_email", "Invalid email format"),
})
```
...
Only middlewares registered with an ID can be removed:

```go
// register middleware with ID
...
// remove the middleware from a specific route
...
Generate a standard auth response with token:
...
Apply expand relations and respect API rules visibility:

```go
// enrich records before returning to client
