...
You can manage your **collections** from the Dashboard, with the Web APIs using the [client-side SDKs](/en/how-to-use) (*superusers only*) or programmatically via the [Go](/en/go/migrations)/[JavaScript](/en/js/migrations) migrations.

Similarly, you can manage your **records** from the Dashboard, with the Web APIs using the [client-side SDKs](/en/how-to-use) or programmatically via the [Go](/en/go/records)/[JavaScript](/en/js/records) Record operations.
...
Dates are compared as strings, meaning that when using the filters with a date field you'll have to specify the full datetime string format. For example to target a single day (e.g. November 19, 2024) you can use something like:
```
...
You can find more detailed information in the [Files upload and handling](/en/files-handling) guide.
