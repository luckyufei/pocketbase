...
| **migrations** | `_ "github.com/pocketbase/pocketbase/migrations"` | System table migrations | `_secrets` tables |
...
| **jobs** | `github.com/pocketbase/pocketbase/plugins/jobs` | 后台任务队列 | `_jobs` 表、延迟任务、失败重试、崩溃恢复 |
...
- **Missing system tables** (`_secrets`): Add `_ "github.com/pocketbase/pocketbase/migrations"`
...
- **Jobs API 不可用**: Add `"github.com/pocketbase/pocketbase/plugins/jobs"` and call `jobs.MustRegister(app, jobs.DefaultConfig())`
