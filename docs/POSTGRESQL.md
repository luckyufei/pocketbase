...
- [Docker 部署](#docker-部署)
- [连接池配置](#连接池配置)
- [PostgreSQL 特性](#postgresql-特性)
- [从 SQLite 迁移](#从-sqlite-迁移)
- [性能优化](#性能优化)
- [故障排除](#故障排除)
- [注意事项](#注意事项)
...
> **注意**：PostgreSQL 模式下 `--auxMaxOpenConns` 和 `--auxMaxIdleConns` 参数会被忽略，因为辅助数据库共享主数据库连接池。
...
**重要**：确保 PostgreSQL 服务器的 `max_connections` 设置大于 `dataMaxOpenConns`。
```
