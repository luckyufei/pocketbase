"""AsyncPocketBase Client - Async version of the SDK."""
...
from pocketbase.services.async_record_service import AsyncRecordService
...
class AsyncPocketBase:
    """Async PocketBase API client.
    
    Async version of the PocketBase client for use with asyncio.
    ...
    def __init__(
        ...
        """Initialize the async PocketBase client.
        ...
        # Service cache
        self._record_services: dict[str, AsyncRecordService] = {}
        ...
        self._realtime_service: AsyncRealtimeService | None = None
    ...
    async def __aenter__(self) -> "AsyncPocketBase":
    ...
    @property
    def realtime(self) -> "AsyncRealtimeService":
        """Get the AsyncRealtimeService for SSE subscriptions."""
        if self._realtime_service is None:
            from pocketbase.services.async_realtime_service import AsyncRealtimeService
            self._realtime_service = AsyncRealtimeService(self)
    ...
    def auto_cancellation(self, enable: bool) -> "AsyncPocketBase":
    ...
    def cancel_request(self, request_key: str) -> "AsyncPocketBase":
    ...
    def cancel_all_requests(self) -> "AsyncPocketBase":
    ...
    async def send(
        ...
        """Send an async HTTP request to the PocketBase API.
    ...
    def collection(self, name: str) -> AsyncRecordService:
        """Get an AsyncRecordService for a collection.
        
        Args:
            name: The collection name.
            
        Returns:
            AsyncRecordService for the collection.
        """
        if name not in self._record_services:
            self._record_services[name] = AsyncRecordService(self, name)
