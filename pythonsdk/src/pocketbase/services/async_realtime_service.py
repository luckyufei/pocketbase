"""AsyncRealtimeService - 异步 SSE 实时订阅服务

提供 PocketBase 异步实时订阅功能，基于 Server-Sent Events (SSE)。
"""
...
SubscriptionCallback = Union[SyncCallback, AsyncCallback]

# 取消订阅函数类型 - 异步版本
UnsubscribeFunc = Callable[[], Coroutine[Any, Any, None]]


class AsyncRealtimeService(BaseService):
    """异步 SSE 实时订阅服务"""

    def __init__(self, client: "AsyncPocketBase") -> None:
    ...
    async def subscribe(
        ...
        if not self.is_connected:
            ...
            await self._submit_subscriptions()

        # 返回取消订阅函数
        async def unsubscribe() -> None:
            await self._unsubscribe_by_topic_and_listener(topic, callback)
    ...
    async def unsubscribe_by_prefix(self, key_prefix: str) -> None:
        """按前缀取消订阅
    ...
    async def _unsubscribe_by_topic_and_listener(
        self,
        topic: str,
    ...
    async def _connect(self) -> None:
        ...
        try:
            client_id = await self._create_sse_connection()
    ...
    async def _create_sse_connection(self) -> str:
        """创建异步 SSE 连接 (实际实现需要 httpx-sse 或 aiohttp)
        ...
        # 这个方法会在实际使用时被真正实现
        # 目前作为占位符，用于测试
        raise NotImplementedError("Async SSE connection not implemented yet")

    async def _disconnect(self, from_reconnect: bool = False) -> None:
        """异步断开 SSE 连接
    ...
    async def _submit_subscriptions(self) -> None:
        """异步提交订阅到服务器"""
