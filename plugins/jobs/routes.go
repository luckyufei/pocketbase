...
func registerRoutes(r *router.Router[*core.RequestEvent], app core.App, config Config) {
	// 创建 /api/jobs 路由组
	jobsGroup := r.Group("/api/jobs")

	// 默认要求超级用户权限
	jobsGroup.Bind(apis.RequireSuperuserAuth())

	// 任务入队
	jobsGroup.POST("/enqueue", jobEnqueueHandler(app, config))
...
func jobEnqueueHandler(app core.App, config Config) func(e *core.RequestEvent) error {
	return func(e *core.RequestEvent) error {
		...
		if len(config.AllowedTopics) > 0 {
			...
			if !allowed {
				return e.BadRequestError("Topic not allowed", nil)
			}
		}
		...
		if req.RunAt != nil {
			...
			job, err = store.EnqueueWithOptions(req.Topic, req.Payload, opts)
		} else if req.MaxRetries > 0 {
			job, err = store.EnqueueWithOptions(req.Topic, req.Payload, opts)
		} else {
			job, err = store.Enqueue(req.Topic, req.Payload)
		}

		if err != nil {
			if err == ErrJobPayloadTooLarge {
...
func jobRequeueHandler(app core.App) func(e *core.RequestEvent) error {
	return func(e *core.RequestEvent) error {
		...
		job, err := store.Requeue(id)
		if err != nil {
			if err == ErrJobNotFound {
				return e.NotFoundError("Job not found", nil)
			}
			if err == ErrJobCannotRequeue {
