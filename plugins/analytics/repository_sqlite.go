...
// RepositorySQLite 实现 SQLite 模式的分析数据存储。
type RepositorySQLite struct {
	db dbx.Builder
}

// NewRepositorySQLite 创建一个新的 SQLite Repository。
func NewRepositorySQLite(db dbx.Builder) *RepositorySQLite {
...
func (r *RepositorySQLite) UpsertSource(ctx context.Context, stat *SourceStat) error {
	now := time.Now().UTC().Format("2006-01-02 15:04:05.000Z")
...
func (r *RepositorySQLite) UpsertDevice(ctx context.Context, stat *DeviceStat) error {
	now := time.Now().UTC().Format("2006-01-02 15:04:05.000Z")
...
func (r *RepositorySQLite) GetDailyStats(ctx context.Context, startDate, endDate string) ([]*DailyStat, error) {
	var stats []*DailyStat
...
func (r *RepositorySQLite) GetTopPages(ctx context.Context, startDate, endDate string, limit int) ([]*DailyStat, error) {
	if limit <= 0 {
		limit = 10
	}
...
func (r *RepositorySQLite) GetTopSources(ctx context.Context, startDate, endDate string, limit int) ([]*SourceStat, error) {
	if limit <= 0 {
		limit = 10
	}
...
func (r *RepositorySQLite) GetDeviceStats(ctx context.Context, startDate, endDate string) ([]*DeviceStat, error) {
...
func (r *RepositorySQLite) GetDailyHLLSketches(ctx context.Context, startDate, endDate string) ([][]byte, error) {
	var results []struct {
		TotalUV []byte `db:"total_uv"`
	}
...
func (r *RepositorySQLite) DeleteBefore(ctx context.Context, date string) error {
