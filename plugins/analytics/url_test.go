...
// TestNormalizeURLSimple 测试简单 URL 规范化（fallback 逻辑）
func TestNormalizeURLSimple(t *testing.T) {
	tests := []struct {
		name string
		url  string
		want string
		...
		{"with protocol and path", "http://example.com/path", "/path"},
		{"with query", "http://example.com/path?q=test", "/path"},
		{"with hash", "http://example.com/path#section", "/path"},
		{"complex", "https://example.com:8080/api/v1?key=value#hash", "/api/v1"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := normalizeURLSimple(tt.url)
			if got != tt.want {
				t.Errorf("normalizeURLSimple(%q) = %q, want %q", tt.url, got, tt.want)
			}
		})
	}
}

// TestExtractQueryInvalid 测试无效 URL 的查询提取
func TestExtractQueryInvalid(t *testing.T) {
	// 一个无法解析的 URL
	got := ExtractQuery("://invalid")
	if got != "" {
		t.Errorf("ExtractQuery with invalid URL = %q, want empty", got)
	}
}

// TestExtractReferrerDomainInvalid 测试无效 URL 的 referrer 提取
func TestExtractReferrerDomainInvalid(t *testing.T) {
	got := ExtractReferrerDomain("://invalid")
	if got != "unknown" {
		t.Errorf("ExtractReferrerDomain with invalid URL = %q, want 'unknown'", got)
	}
}
