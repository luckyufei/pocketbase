...
// - {secret.VAR_NAME}: 从 secrets 获取（通过 SecretGetter）
// - @request.auth.field: 从当前认证用户读取字段
func ParseHeaderTemplate(template string, authInfo *AuthInfo, secretGetter SecretGetter) (string, error) {
	if template == "" {
		return "", nil
	}
	...
	result, err = replaceSecretVars(result, secretGetter)
	if err != nil {
		return "", err
	}

	// 3. 替换认证上下文 @request.auth.*
	result = replaceAuthVars(result, authInfo)
...
func replaceSecretVars(template string, secretGetter SecretGetter) (string, error) {
	...
	result := secretVarRegex.ReplaceAllStringFunc(template, func(match string) string {
		...
		// 尝试从 secrets 获取
		if secretGetter != nil {
			secret, err := secretGetter(varName)
			if err == nil && secret != "" {
				return secret
			}
		}
...
func replaceAuthVars(template string, authInfo *AuthInfo) string {
	if authInfo == nil {
	...
	return authVarRegex.ReplaceAllStringFunc(template, func(match string) string {
		...
		// 获取其他字段
		value := authInfo.GetField(fieldName)
		if value == nil {
			return ""
		}
...
// 解析所有模板并返回最终的请求头映射
func BuildProxyHeaders(headers map[string]string, authInfo *AuthInfo, secretGetter SecretGetter) (map[string]string, error) {
	result := make(map[string]string, len(headers))

	for key, template := range headers {
		value, err := ParseHeaderTemplate(template, authInfo, secretGetter)
		if err != nil {
