...
func TestFileFieldIntercept(t *testing.T) {
	tests.DualDBTest(t, func(t *testing.T, app *tests.TestApp, dbType tests.DBType) {
		...
		ok = t.Run("2. create - fixing the validation error", func(t *testing.T) {
			...
			checkRecordFiles(t, app, record, expectedKeys)
		})
		if !ok {
			return
		}
		...
		ok = t.Run("3. update - validation error", func(t *testing.T) {
			...
			checkRecordFiles(t, app, record, []string{f1.Name, f2.Name})
		})
		if !ok {
			return
		}
		...
		ok = t.Run("4. update - fixing the validation error", func(t *testing.T) {
			...
			checkRecordFiles(t, app, record, []string{f1.Name, f3.Name})
		})
		if !ok {
			return
		}
		...
		t.Run("5. update - second time update", func(t *testing.T) {
			...
			checkRecordFiles(t, app, record, []string{f3.Name, f4.Name})
		})
	})
}
...
func TestFileFieldInterceptTx(t *testing.T) {
	tests.DualDBTest(t, func(t *testing.T, app *tests.TestApp, dbType tests.DBType) {
		...
		// failed transaction
		txErr := app.RunInTransaction(tx(false))
		if txErr == nil {
			t.Fatal("Expected transaction error")
		}
		// there shouldn't be any fails associated with the record id
		checkRecordFiles(t, app, record, []string{})

		txErr = app.RunInTransaction(tx(true))
		if txErr != nil {
