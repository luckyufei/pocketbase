...
// AnalyticsRepositoryPostgres 实现 PostgreSQL 模式的分析数据存储。
// 与 SQLite 版本的主要区别：
// - 使用 UNLOGGED 表提高写入性能
// - 使用 ON CONFLICT 语法实现 upsert
type AnalyticsRepositoryPostgres struct {
	db dbx.Builder
}

// NewAnalyticsRepositoryPostgres 创建一个新的 PostgreSQL Repository。
func NewAnalyticsRepositoryPostgres(db dbx.Builder) *AnalyticsRepositoryPostgres {
	return &AnalyticsRepositoryPostgres{db: db}
}

// UpsertDaily 更新或插入每日统计数据。
// 使用 PostgreSQL 的 ON CONFLICT 语法实现原子 upsert。
func (r *AnalyticsRepositoryPostgres) UpsertDaily(ctx context.Context, stat *AnalyticsDailyStat) error {
	now := time.Now().UTC().Format("2006-01-02 15:04:05.000Z")

	// 先尝试读取现有记录以合并 HLL
	var existing AnalyticsDailyStat
	err := r.db.Select("*").
...
func (r *AnalyticsRepositoryPostgres) UpsertSource(ctx context.Context, stat *AnalyticsSourceStat) error {
	now := time.Now().UTC().Format("2006-01-02 15:04:05.000Z")

	var existing AnalyticsSourceStat
	err := r.db.Select("*").
...
func (r *AnalyticsRepositoryPostgres) UpsertDevice(ctx context.Context, stat *AnalyticsDeviceStat) error {
	now := time.Now().UTC().Format("2006-01-02 15:04:05.000Z")

	var existing AnalyticsDeviceStat
	err := r.db.Select("*").
...
func (r *AnalyticsRepositoryPostgres) GetDailyStats(ctx context.Context, startDate, endDate string) ([]*AnalyticsDailyStat, error) {
...
func (r *AnalyticsRepositoryPostgres) GetTopPages(ctx context.Context, startDate, endDate string, limit int) ([]*AnalyticsDailyStat, error) {
	if limit <= 0 {
		limit = 10
	}
...
func (r *AnalyticsRepositoryPostgres) GetTopSources(ctx context.Context, startDate, endDate string, limit int) ([]*AnalyticsSourceStat, error) {
	if limit <= 0 {
		limit = 10
	}
...
func (r *AnalyticsRepositoryPostgres) GetDeviceStats(ctx context.Context, startDate, endDate string) ([]*AnalyticsDeviceStat, error) {
...
func (r *AnalyticsRepositoryPostgres) GetDailyHLLSketches(ctx context.Context, startDate, endDate string) ([][]byte, error) {
	var results []struct {
...
func (r *AnalyticsRepositoryPostgres) DeleteBefore(ctx context.Context, date string) error {
...
// Close 关闭存储连接。
func (r *AnalyticsRepositoryPostgres) Close() error {
	// PostgreSQL 使用共享的 App DB，不需要单独关闭
	return nil
}

// 确保实现了接口
var _ AnalyticsRepository = (*AnalyticsRepositoryPostgres)(nil)
