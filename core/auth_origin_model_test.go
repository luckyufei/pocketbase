...
func TestAuthOriginPasswordChangeDeletion(t *testing.T) {
	...
	scenarios := []struct {
		...
		{
			"1_superusers_with_auth_origins",
			func(app *tests.TestApp) (*core.Record, error) {
				return app.FindAuthRecordByEmail(core.CollectionNameSuperusers, "test2@example.com")
			},
			[]string{"5798yh833k6w6w0", "ic55o70g4f8pcl4", "dmy260k6ksjr4ib"},
		},
		{
			"2_clients_with_auth_origins",
			func(app *tests.TestApp) (*core.Record, error) {
				return app.FindAuthRecordByEmail("clients", "test@example.com")
			},
			[]string{"9r2j0m74260ur8i"},
		},
	}
