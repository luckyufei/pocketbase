...
type AnalyticsBuffer struct {
	...
	sourceAggregations map[string]*AnalyticsSourceAggregation

	// deviceAggregations 存储按 date+browser+os 聚合的数据
	deviceAggregations map[string]*AnalyticsDeviceAggregation
}

// NewAnalyticsBuffer 创建一个新的缓冲区。
func NewAnalyticsBuffer(maxRawSize int) *AnalyticsBuffer {
	if maxRawSize <= 0 {
	...
	return &AnalyticsBuffer{
		...
		rawBuffer:          make([]*AnalyticsEvent, 0, 1024),
		aggregations:       make(map[string]*AnalyticsAggregation),
		sourceAggregations: make(map[string]*AnalyticsSourceAggregation),
		deviceAggregations: make(map[string]*AnalyticsDeviceAggregation),
	}
}

// Push 将事件推入缓冲区。
// 事件会同时进入 Raw Buffer 和 Aggregation Map（Fork）。
func (b *AnalyticsBuffer) Push(event *AnalyticsEvent) error {
	if event == nil {
		return nil
	}
...
func (b *AnalyticsBuffer) RawSize() int {
	b.mu.RLock()
	defer b.mu.RUnlock()
	return b.rawSize
}
...
func (b *AnalyticsBuffer) ShouldFlushRaw() bool {
	b.mu.RLock()
	defer b.mu.RUnlock()
...
func (b *AnalyticsBuffer) DrainRaw() []*AnalyticsEvent {
	b.mu.Lock()
	defer b.mu.Unlock()

	events := b.rawBuffer
	b.rawBuffer = make([]*AnalyticsEvent, 0, 1024)
	b.rawSize = 0
...
func (b *AnalyticsBuffer) DrainAggregations() map[string]*AnalyticsAggregation {
	b.mu.Lock()
	defer b.mu.Unlock()

	aggs := b.aggregations
	b.aggregations = make(map[string]*AnalyticsAggregation)

	return aggs
}

// DrainSourceAggregations 取出并清空 Source Aggregation Map。
func (b *AnalyticsBuffer) DrainSourceAggregations() map[string]*AnalyticsSourceAggregation {
	b.mu.Lock()
	defer b.mu.Unlock()

	aggs := b.sourceAggregations
	b.sourceAggregations = make(map[string]*AnalyticsSourceAggregation)

	return aggs
}

// DrainDeviceAggregations 取出并清空 Device Aggregation Map。
func (b *AnalyticsBuffer) DrainDeviceAggregations() map[string]*AnalyticsDeviceAggregation {
	b.mu.Lock()
	defer b.mu.Unlock()

	aggs := b.deviceAggregations
	b.deviceAggregations = make(map[string]*AnalyticsDeviceAggregation)

	return aggs
}

// AggregationCount 返回聚合条目数量。
func (b *AnalyticsBuffer) AggregationCount() int {
	b.mu.RLock()
	defer b.mu.RUnlock()
...
func (b *AnalyticsBuffer) updateAggregation(event *AnalyticsEvent) {
...
func (b *AnalyticsBuffer) updateSourceAggregation(event *AnalyticsEvent) {
	if event.Referrer == "" {
		return
	}
	...
	if !exists {
		agg = &AnalyticsSourceAggregation{
			Date:   date,
			Source: source,
		}
...
func (b *AnalyticsBuffer) updateDeviceAggregation(event *AnalyticsEvent) {
	date := event.Timestamp.Format("2006-01-02")
	...
	if !exists {
		agg = &AnalyticsDeviceAggregation{
			Date:    date,
			Browser: browser,
			OS:      os,
		}
...
func (b *AnalyticsBuffer) estimateEventSize(event *AnalyticsEvent) int {
	// 粗略估算：固定开销 + 字符串长度
...
func (b *AnalyticsBuffer) RestoreAggregations(aggs map[string]*AnalyticsAggregation) {
	if len(aggs) == 0 {
		return
	}
...
func (b *AnalyticsBuffer) RestoreSourceAggregations(aggs map[string]*AnalyticsSourceAggregation) {
	if len(aggs) == 0 {
		return
	}
...
func (b *AnalyticsBuffer) RestoreDeviceAggregations(aggs map[string]*AnalyticsDeviceAggregation) {
	if len(aggs) == 0 {
		return
	}
