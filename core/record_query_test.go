...
func TestFindAuthRecordByEmail(t *testing.T) {
	scenarios := []struct {
		...
		// 当 nocaseIndex=false 时，大小写不同应找不到记录
		// 当 nocaseIndex=true 时，应能找到记录（SQLite 用 COLLATE NOCASE，PostgreSQL 用 LOWER()）
		{"clients", "TeSt2@example.com", false, true},
		{"clients", "TeSt2@example.com", true, false},
	}

	for _, s := range scenarios {
		tests.RunWithBothDBs(t, fmt.Sprintf("%s_%s", s.collectionIdOrName, s.email), func(t *testing.T, app *tests.TestApp, dbType tests.DBType) {
			...
			if collection != nil {
				...
				if ok {
					// 为测试设置 nocase collation（两种数据库都支持）
					if s.nocaseIndex {
