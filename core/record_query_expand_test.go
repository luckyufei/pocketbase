...
func TestExpandRecords(t *testing.T) {
	...
	tests.DualDBTest(t, func(t *testing.T, app *tests.TestApp, dbType tests.DBType) {
		scenarios := []struct {
			...
			recordIds                 []string
			expands                   []string
			fetchFunc                 func(app *tests.TestApp) core.ExpandFetchFunc
		...
		}{
			{
				...
				func(app *tests.TestApp) core.ExpandFetchFunc {
					return func(c *core.Collection, ids []string) ([]*core.Record, error) {
		...
		for _, s := range scenarios {
			t.Run(s.testName, func(t *testing.T) {
				...
				var fetchFunc core.ExpandFetchFunc
				if s.fetchFunc != nil {
					fetchFunc = s.fetchFunc(app)
				}

				failed := app.ExpandRecords(records, s.expands, fetchFunc)
...
func TestExpandRecord(t *testing.T) {
	...
	tests.DualDBTest(t, func(t *testing.T, app *tests.TestApp, dbType tests.DBType) {
		scenarios := []struct {
			...
			{
				"nested indirect expand via multiple relation field",
				"demo3",
				"lcl9d87w22ml6jy",
				[]string{
					"demo4_via_rel_many_no_cascade_required.self_rel_many.rel_many_no_cascade_required.demo4_via_rel_many_no_cascade_required",
		...
		for _, s := range scenarios {
			t.Run(s.testName, func(t *testing.T) {
				...
				failed := app.ExpandRecord(record, s.expands, fetchFunc)
