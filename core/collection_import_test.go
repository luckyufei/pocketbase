...
func TestImportCollections(t *testing.T) {
	...
	tests.DualDBTest(t, func(t *testing.T, app *tests.TestApp, dbType tests.DBType) {
		...
		err := app.CollectionQuery().AndWhere(dbx.HashExp{"system": false}).All(&regularCollections)
		if err != nil {
			t.Fatal(err)
		}

		var systemCollections []*core.Collection
		err = app.CollectionQuery().AndWhere(dbx.HashExp{"system": true}).All(&systemCollections)
		if err != nil {
			t.Fatal(err)
		}
		...
		for _, s := range scenarios {
			t.Run(s.name, func(t *testing.T) {
				// Create a fresh app for each sub-test to avoid state pollution
...
func TestImportCollectionsCreateRules(t *testing.T) {
	...
	tests.DualDBTest(t, func(t *testing.T, app *tests.TestApp, dbType tests.DBType) {
		err := app.ImportCollections([]map[string]any{
		...
		collection, err := app.FindCollectionByNameOrId("new_test")
		if err != nil {
			t.Fatal(err)
		}
